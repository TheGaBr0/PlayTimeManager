    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String s, @NotNull String[] args) {
        if (sender.hasPermission("playtime.percentage")) {
            if(args.length>0){
                ...
                sender.sendMessage("[§6PlayTime§eManager§f]§7 The players with playtime greater than or equal to §6" + args[0] +
                        " §7are §6" + result[1] + " §7and represent §6"+ formattedNumber +"% §7of the §6"+ result[2] +" §7players stored");
            }else{
                sender.sendMessage("[§6PlayTime§eManager§f]§7 Missing arguments");
            }
        } else {
